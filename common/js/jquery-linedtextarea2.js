...
(function($) {
  ...
  $.fn.linedtextarea = function(options) {
    ...
    var fillOutLines = function(codeLines, h, lineNo){
      while ((codeLines.height() - h) <= 0) {
        if (lineNo == opts.selectedLine)
          codeLines.append("<div class='lineno lineselect'>" +
			   lineNo +
			   "</div>");
        else
          codeLines.append("<div class='lineno lineno-" + lineNo + "'>" +
			   lineNo +
			   "</div>");
        lineNo++;
      }
	...
	"<div class='linedwrap' style='width:" +
      ...
      lineNo = fillOutLines(codeLinesDiv, linesDiv.height(), opts.startAt);

      /* Move the textarea to the selected line */
      if (opts.selectedLine != -1 && !isNaN(opts.selectedLine)){
        var fontSize = parseInt(textarea.height() / (lineNo-2));
        var position = parseInt(fontSize * opts.selectedLine) -
	    (textarea.height()/2);
	  ...
	  sidebarWidth - paddingHorizontal - 20;
