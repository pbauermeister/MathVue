function DropboxManager(
  ...
  if (this.dropboxStorage.isLoggedIn()) {
    ...
    this.dropboxStorage.loginIfNeeded(
  ...
  this.dropboxSaveDialog = function () {
    ...
    this.dropboxStorage.listFolder(
      null,
      function (entries) {
        busy.close();
        entries = this._dropboxFilterEntries(entries);
        this.fileDialog.saveFile(
          initial_filename,
          entries,
          this.dropboxSaveFile,
          this.dropboxStorage,
          function (filename) {
  ...
  this.dropboxLoadDialog = function () {
    ...
    this.dropboxStorage.listFolder(
      ...
      function (entries) {
        busy.close();
        entries = this._dropboxFilterEntries(entries);
        this.fileDialog.openFile(
          entries,
          this.dropboxLoadFile,
          this.dropboxStorage
        );
      }.bind(this),
      function (error) {
  ...
  this.dropboxLoadFile = function (entry) {
    ...
    this.dropboxStorage.downloadFile(
      entry.id,
      function (data) {
        busy.close();
        setFormulaCB(data);
  ...
  this.dropboxLoadFileGallery = function (entry, then) {
    var busy = this.fileDialog.showBusyDialog("Loading file...");
    this.dropboxStorage.downloadFilePublic(
      entry.formula_url,
      function (data) {
        busy.close();
